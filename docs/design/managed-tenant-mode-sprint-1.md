...
Not in sprint 1:
- Principal/product/creative/workflow API endpoints (publisher-managed; the proxied UI handles these — they're sprint 2 as automation conveniences, not blockers).
- Read-only operations console endpoints (status, workflows list, media-buys, audit-log) — sprint 5.
- UI hardening / banner / nav hiding — sprint 4.
- Network-policy lockdown for `MANAGED_INSTANCE` — sprint 4.

...
- Buyer protocol endpoints (`/mcp/`, `/a2a`) accept traffic only from the configured private network range (configurable via `BUYER_PROTOCOL_ALLOWED_CIDR` or similar). Public traffic gets `403`.
- Tenant Management API (`/api/v1/tenant-management`) same network restriction.
...
Net result: zero public exposure for the entire salesagent on a managed instance.
...
| Sprint | Deliverable |
|---|---|
| 1 | **Full platform-managed surface via API.** Migrations (`managed_externally`, `external_org_id`, `external_source` on Tenant; external identity fields on AuditLog). `MANAGED_INSTANCE` env. Scoped write guard at the model layer. spectree wired up. Tenant lifecycle endpoints (provision, list, get, patch, deactivate, reactivate, delete). Adapter management endpoints (get, put, test-connection). Identity-header reader middleware. Reverse-proxy compatibility verified. Swagger UI live. *After this sprint, Scope3 can fully manage tenants via API.* |
| 2 | **Publisher-managed surfaces via API** (automation/bulk-management; UI also covers these). Principal CRUD + token rotation. Product CRUD + autogenerate-from-GAM. Lets Scope3 bulk-manage advertisers/inventory if it wants. |
| 3 | Remaining publisher-managed sub-resources via API (properties, profiles, slack, business rules, agents, policy). Full API surface for both platform and publisher scopes. |
| 4 | UI hardening: middleware that scopes nav by `managed_externally`, hides platform-config pages, renders banners on platform-config pages, leaves publisher pages fully writable. Network policy for `MANAGED_INSTANCE`. |
| 5 | Status + workflows + media-buys + audit-log read endpoints via API. Scope3 surfaces salesagent state in Storefront overview. |
| 6 (optional) | Outbound webhooks. |

Sprints 1–2 unblock the Scope3 integration end-to-end. Sprints 3–5 complete the surface. Each sprint is independently shippable.

## Implementation notes

The 23 admin UI blueprints contain ~693 inline DB access patterns and only 4 references to `_impl()`-style transport-agnostic functions. The Tenant Management API endpoints will need that business logic extracted.

**Recommended path:** greenfield extraction. Write new `_impl()` functions and Pydantic schemas for each Management API endpoint. UI handlers continue using their inline DB code on open instances. Both paths converge over time as UI handlers are opportunistically refactored to delegate.

**Why not refactor first:** would block API delivery on a multi-sprint refactor that doesn't ship value to Scope3 until the end.
...
- **Two control planes during transition.** Until middleware ships in sprint 4, both Scope3 and any direct admin-UI access can write to managed tenants. Mitigation: ship middleware in sprint 1 alongside the flag, even before all API endpoints exist.

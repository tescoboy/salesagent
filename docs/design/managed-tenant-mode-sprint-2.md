# Sprint 2 Spec: Publisher-Managed CRUD via API

**Parent design:** [managed-tenant-mode.md](./managed-tenant-mode.md)
**Builds on:** [managed-tenant-mode-sprint-1.md](./managed-tenant-mode-sprint-1.md)
**Status:** Draft
**Last updated:** 2026-05-04

## Scope

Sprint 2 expands the Tenant Management API into the **publisher-managed scope**: principals (advertisers) and products. These surfaces are also editable from the proxied UI, so the API is an automation/bulk-management convenience — not a blocker for the Scope3 integration. They're built in sprint 2 because:
...
Token management is intentionally not part of the Tenant Management API. Auth is handled at the platform edge — Scope3 issues credentials to its customers and runs its own buyer agent against the salesagent. Per-principal tokens (today's `x-adcp-auth` mechanism) are an open-instance concern.

This means:
- `POST /principals` does not return an API token. Principals are pure advertiser-identity records in managed mode.
...
**Dependency for end-to-end functionality**: Scope3's buyer agent still needs to call MCP/A2A on behalf of a specific advertiser. In managed mode, that auth flows through the identity-propagation contract rather than per-principal tokens — likely a `X-Principal-Id` header alongside `X-Identity-*`, trusted because the network is private. Designing the buyer-protocol auth path for managed mode is its own work, tracked separately from this sprint. Sprint 2 deliberately scopes tokens out of the API.
...
- Workflow approval API (sprint 5 — read-only ops console)
- Authorized properties / inventory profiles / business rules / agents / policy (sprint 3)
...
Products are heavily AdCP-spec-driven. The library's `Product` type carries most fields. Sprint 2 schemas extend it minimally:
...
No token is generated or returned. Buyer-protocol auth in managed mode flows through the identity-propagation contract; per-principal tokens are not part of the managed-mode API surface.
...
The marquee endpoint of sprint 2. Bootstraps a product catalog from the publisher's GAM inventory.
...
Sprint 1 established a partial `_impl()` extraction pattern. Sprint 2 adds two new repositories that the API endpoints consume — and that the existing UI handlers can opportunistically migrate to:
...
The existing UI blueprints (`src/admin/blueprints/principals.py`, `src/admin/blueprints/products.py`) still use inline DB code. Sprint 2 does **not** require migrating them — that's opportunistic future work. The structural-guard FIXME allowlist tracks the pre-existing violations.
...
- [ ] After sprint 1 + sprint 2: provision a managed tenant, autogenerate products from GAM, create a principal — entire flow works via API only without touching the UI.
- [ ] After sprint 1 + sprint 2: existing UI handlers for principals/products on a managed tenant still work — the model write guard does not fire on these tables.
...
1. **Buyer-protocol auth for managed mode.** With principal tokens out of the API surface, MCP/A2A in managed mode needs a different auth path. Likely candidate: extend the identity-propagation contract with `X-Principal-Id` (and possibly `X-Tenant-Id` if not derivable from `X-Identity-Org-Id`), trusted because the network is private. Specify this in a sibling design doc before sprint 2 lands — the salesagent's MCP/A2A endpoints need to know how to scope a call without a bearer token in managed mode.
2. **Existing Principal.token column.** Today's `Principal` model has a token field used by open-instance MCP/A2A. In managed mode it's unused. Sprint 2 leaves it in place (open instances still depend on it); the column is simply not populated for managed-tenant principals. Document this clearly so future readers don't assume the field is dead.
...
5. **Bulk principal creation.** Sprint 2 ships single-create. If Scope3 needs to onboard hundreds at a time, add `POST /principals/bulk` accepting a list. Defer until Scope3 confirms.

## What sprint 3+ builds on this

With sprint 1 (platform) + sprint 2 (publisher CRUD) shipped, Scope3 has full programmatic control over a managed tenant. Sprint 3 fills out the remaining publisher-managed sub-resources via API (properties, profiles, slack, business rules, agents, policy) — same plumbing, different tables. Sprint 4 hardens the UI for managed tenants. Sprint 5 adds read-only operations endpoints.

...
- Hide the multi-currency add/remove UI on managed tenants.
...
These remain editable for managed tenants — they're publisher business logic,
...
- Reuse the existing `templates/_managed_mode_banner.html` partial if one
...
callers on managed tenants (sprint 1.8 §6 first half — `CurrencyLimit`
...
### Hidden pages
- [ ] `/tenant/{id}/users` on a managed tenant returns 200 with the
...
- [ ] Settings page on a managed tenant omits the Account → Organization
...
- [ ] Policies & Workflows tab fully editable on managed tenants.
- [ ] Publisher Partnerships, Products, Advertisers, Integrations all
      editable on managed tenants.
- [ ] Inventory Sync Status is read-only on managed tenants but visible.

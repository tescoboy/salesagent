"""Integration tests: creative formats protocol and MCP transport.

Tests MCP combined-filter dispatch, MCP ToolResult wrapping, full catalog
via _impl, and tenant context resolution from identity.

Obligation IDs:
- UC-005-MAIN-MCP-16: combined filters narrow results
- UC-005-MAIN-MCP-17: MCP ToolResult wrapping
- UC-005-MAIN-REST-01: full catalog via _impl
- UC-005-MAIN-REST-03: tenant context from identity
"""
...
class TestFullCatalog:
    """Covers: UC-005-MAIN-REST-01 -- _impl returns complete format catalog."""

    def test_returns_complete_catalog(self, integration_db):
        """UC-005-MAIN-REST-01: _impl returns full catalog of registered formats."""
        formats = [
    ...
    def test_response_format_structure(self, integration_db):
        """UC-005-MAIN-REST-01: each format in response has required fields.
    ...
    def test_empty_catalog_returns_empty_formats(self, integration_db):
...
class TestTenantContextFromIdentity:
    """Covers: UC-005-MAIN-REST-03 -- tenant context resolved from identity."""

    def test_uses_tenant_from_identity(self, integration_db):
        """UC-005-MAIN-REST-03: _impl resolves tenant from ResolvedIdentity."""
        ...
        with CreativeFormatsEnv(tenant_id="my_tenant") as env:
            ...
            response = env.call_impl(identity=identity)

        assert isinstance(response, ListCreativeFormatsResponse)
        assert len(response.formats) == 1

    def test_different_tenants_get_different_catalogs(self, integration_db):
        ...
        with CreativeFormatsEnv(tenant_id="tenant_a") as env_a:
            ...
            response_a = env_a.call_impl(identity=identity_a)
        ...
        with CreativeFormatsEnv(tenant_id="tenant_b") as env_b:
            ...
            response_b = env_b.call_impl(identity=identity_b)
